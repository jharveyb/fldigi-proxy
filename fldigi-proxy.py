...
class fl_instance:
    ...
    async def radio_send(self, tx_msg):
        ...
        #     print(f"Sent: {tx_confirm_fragment}")
    ...
    async def radio_receive(self):
        ...
        while True:
            ...
            #     if not rx_prefix_check:
            #         rx_prefix += rx_fragment
            #         if rx_prefix.endswith(self.base64_prefix):
            #             rx_prefix_check = True
            #     # all data before prefix excluded from message, including noise from
            #     # keying up radio
            #     else:
            if rx_fragment.endswith(b"\r\n"):
                print(f"Got fragment: {rx_fragment}")
                rx_msg += rx_fragment
                break
            else:
                rx_msg += rx_fragment
        self.fl_client.text.clear_rx()
        # Hack to strip prefix
        return rx_msg[3:-2]
    ...
    def rig_info(self):
        print(
            "bandwidth",
            self.fl_client.rig.bandwidth,
            "frequency",
            self.fl_client.rig.frequency,
            "mode",
            self.fl_client.rig.mode,
            "name",
            self.fl_client.rig.name,
        )
    ...
    def modem_info(self):
        print(
            "bandwidth",
            self.fl_client.modem.bandwidth,
            "carrier",
            self.fl_client.modem.carrier,
            "modem",
            self.fl_client.modem.name,
        )
...
async def main():
    ...
    if not args.noproxy and args.proxyport is None:
        print("Need a proxy port!")
        return

    fl_main = fl_instance(
        no_daemon=args.nodaemon,
        no_proxy=args.noproxy,
        xml_port=args.xml,
        proxy_port=args.proxyport,
        headless=args.nohead,
    )
